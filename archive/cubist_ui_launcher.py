...
def run_wrapper():
    ...
    try:
        ...
        clip = clip_var.get()
        run_cubist(input_path, output_dir, total_points=total_points, clip_to_alpha=clip)
...
tk.Button(root, text="Run Cubist Generator", command=run_wrapper).grid(row=4, column=1, pady=10)
