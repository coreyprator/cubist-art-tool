...
def generate(
    ...
    **params,
    ...
    while len(rects) < target and iteration_count < max_iterations:
        ...
        splitnable_rects = [(i, r) for i, r in enumerate(rects) if r[2] > min_size and r[3] > min_size]
        ...
        idx, (x, y, w, h) = max(splitnable_rects, key=lambda item: item[1][2] * item[1][3])
        rects.pop(idx)
        ...
        shapes.append(
            {
                ...
                "fill": _sample_image_color(input_image, center_x, center_y, width, height),
                "stroke": (0, 0, 0),
                "stroke_width": 0.5,
            }
        )
