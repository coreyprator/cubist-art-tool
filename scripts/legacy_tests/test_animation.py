...
def main():
    ...
    parser.add_argument('--geometry', choices=['delaunay', 'voronoi', 'rectangles'], required=False)
    parser.add_argument('--points', type=int, default=1000)
    parser.add_argument('--cascade_fill', type=str, default="false", help="Use cascade fill (true/false)")
    parser.add_argument('--cascade-stages', type=int, default=3, help='Number of cascade stages (default 3)')
    parser.add_argument('--verbose', action='store_true')
    parser.add_argument('--export-svg', action='store_true', help='Also export an SVG alongside the PNG')
    ...
    try:
        ...
        produced_png, shapes, (width, height), sampled_points, metrics_dict = run_cubist(
    ...
    if args.export_svg:
        ...
        try:
            ...
            print(f"METRICS geometry={geometry_mode} points={len(sampled_points) if sampled_points is not None else 'NA'} shapes={raster_shapes} svg_path={svg_path}")
    ...
    if metrics_dict and metrics_dict.get("stages"):
        for m in metrics_dict["stages"]:
            print(f"METRICS: stage={m['stage']} geometry={m['geometry_mode']} points={m['points']} svg_shapes={m.get('svg_shape_count')} seed={m.get('seed')}")
        totals = metrics_dict.get("totals", {})
        print(f"METRICS_TOTAL: geometry={totals.get('geometry_mode')} points={totals.get('points')} stages={totals.get('stages')} svg_shapes={totals.get('svg_shape_count')} seed={totals.get('seed')}")
    else:
        print(
            "METRICS: "
    ...
    if args.metrics_json:
        if metrics_dict:
            ...
            metrics_dict["totals"]["svg_shape_count"] = svg_count if final_svg_path else None
    ...
    logger.info(f"cubist_cli.py EXIT with PNG: {final_png}, exists: {exists_flag}{_svg_info}")
