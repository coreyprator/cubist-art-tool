...
if os.path.exists(test_file_local):
    ...
    print(f"   ❌ File does NOT exist despite success return")
...
if os.path.exists(test_file_gdrive):
    ...
    print(f"   ❌ Google Drive file does NOT exist")
...
def create_edge_density_map():
    ...
    output_density = r"G:\My Drive\Code\Python\cubist_art\input\edge_density_your_input_image.png"
    ...
    if density_smooth.max() > density_smooth.min():
        density_norm = (density_smooth - density_smooth.min()) / (density_smooth.max() - density_smooth.min())
    else:
    ...
    if not success:
        print(f"❌ Failed to save density map")
        return False
    ...
    if os.path.exists(output_density):
        ...
        print(f"   ❌ Density map file not found after save")
        return False
    ...
    cv2.putText(legend, "Edge Density Map: Red/Yellow=High Density, Blue=Low Density", 
    ...
    if success_preview and os.path.exists(output_preview):
        ...
        print(f"   ⚠️ Preview creation failed")
