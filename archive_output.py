...
def archive_output_folder(dry_run=False):
    ...
    logger.info(f"Found {len(output_contents)} items to archive: {[item.name for item in output_contents]}")
    ...
    if dry_run:
        ...
        logger.info(f"DRY RUN: Would archive {len(output_contents)} items to {archive_path}")
    ...
    try:
        ...
        if remaining_items:
            logger.warning(f"Output directory not fully cleaned - {len(remaining_items)} items remain")
        ...
        logger.info(f"archive_output_folder() EXIT: Successfully archived to {archive_path}")
