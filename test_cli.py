...
try:
    ...
    try:
        ...
        with open(os.path.join(os.path.dirname(__file__), "logs", "run_log.txt"), "a", encoding="utf-8") as _lf:
...
def archive_output_dir(out_dir: str):
    try:
        ...
        if not p.exists():
            return
        files = [x for x in p.iterdir() if x.is_file() and x.suffix.lower() in {".png", ".svg"}]
        if not files:
            return
...
def run_cubist_cli(input_path: str, output_dir: str, geometry_mode: str, 
                   use_cascade_fill: bool, total_points: int = 1000, 
                   mask_path: str = None, save_step_frames: bool = False,
                   verbose: bool = True, timeout_seconds: int = 300,
    ...
    import threading
    import io
    logger.info(f"run_cubist_cli() ENTRY: mode={geometry_mode}, cascade={use_cascade_fill}, points={total_points}")
    # Prepare output paths
    base_filename = Path(input_path).stem
    png_output_path = Path(output_dir) / f"{base_filename}_{geometry_mode}_{total_points}.png"
    ...
    cli_command = [
        ...
        "--cascade_fill", str(use_cascade_fill),
    ...
    logger.info(f"Running CLI command: {' '.join(str(x) for x in cli_command if x is not None)}")
    logger.info(f"Subprocess start time: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"Timeout seconds: {timeout_seconds}")
    t0 = time.time()
    try:
        import subprocess
        result = subprocess.run(cli_command, capture_output=True, text=True, check=False, timeout=timeout_seconds)
        stdout = result.stdout
        stderr = result.stderr
        # Parse METRICS line
        m = re.search(r"METRICS\s+geometry=(\w+)\s+raster_shapes=(\d+)\s+svg_path=([^\s]+)", stdout)
        if m:
            geom = m.group(1)
            ...
            print(f"[svg-validate] {status} | geometry={geom} | raster={raster_cnt} | svg={svg_cnt} | file={svg_path}")
            if not ok:
                raise SystemExit(f"SVG validation failed for {geom}: raster={raster_cnt}, svg={svg_cnt}")
        else:
            print("[svg-validate] INFO: No METRICS line found; skipping SVG parity check.")
...
def run_single_test(
    ...
    try:
        ...
        svg_out = str(Path(output_dir) / _ts_name(base_stem, geometry_mode, total_points, ".svg")) if export_svg else None
        ...
        if not png_output_path:
            ...
            return None
        logger.info(f"Test completed successfully in {duration:.2f}s: {png_output_path}")
...
Examples:
  ...
  python test_cli.py --mask_only_tests --input input/image.jpg --mask input/mask.png --points 200
    ...
    parser.add_argument('--archive', action='store_true',
                        help='Alias for --archive_output')
    parser.add_argument('--quiet', '-q', type=parse_bool, default=False,
                       help='Reduce output verbosity')
    parser.add_argument('--export-svg', nargs='?', const=True, default=False,
                        help='Export SVG alongside PNG output (optionally specify path)')
    parser.add_argument('--timeout-seconds', type=int, default=300,
                        help='Timeout in seconds for the run (default 300)')
    parser.add_argument('--triple-test', action='store_true', default=False,
    ...
    args.export_svg = args.export_svg if args.export_svg is False or isinstance(args.export_svg, str) else True
    ...
    try:
        if args.run_all_tests:
            ...
            print(f"Starting test: {args.geometry} at {datetime.datetime.now().strftime('%H:%M:%S')}")
            t0 = time.time()
            ...
            with open(os.path.join(os.path.dirname(__file__), "logs", "run_log.txt"), "a", encoding="utf-8") as logf:
                if png_output_path:
                    logf.write(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] SUCCESS: {png_output_path}\n")
                else:
                    logf.write(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] FAILURE: {args.geometry}\n")
            # Log final PNG path and existence
            logger.info(f"Final PNG path: {png_output_path}, exists: {os.path.exists(str(png_output_path)) if png_output_path else False}")
        ...
        with open(log_file_path, "a", encoding="utf-8") as logf:
            logf.write(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] INTERRUPTED\n")
        sys.exit(2)
    except Exception as e:
        ...
        log_file_path = os.path.join(os.path.dirname(__file__), "logs", "run_log.txt")
        with open(log_file_path, "a", encoding="utf-8") as logf:
            logf.write(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ERROR: {str(e)}\n")
